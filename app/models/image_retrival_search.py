...
def search_top_k_return_ids(query_image_path, top_k=100,similarity_threshold=0.8):
    # Load database (có thể cache nếu muốn tăng tốc)
    ...
    similarities = F.cosine_similarity(query_emb.unsqueeze(0), embeddings)
     # Lọc ra những indices có similarity > threshold
    filtered_indices = [i for i, sim in enumerate(similarities) if sim > similarity_threshold]

    # Nếu có nhiều hơn top_k kết quả thỏa điều kiện, lấy top_k có similarity cao nhất
    filtered_indices_sorted = sorted(filtered_indices, key=lambda i: similarities[i], reverse=True)[:top_k]

    result_ids = []
    for idx in filtered_indices_sorted:
        filename = os.path.basename(image_paths[idx])
        products_spu_id = os.path.splitext(filename)[0].split("_")[0]
        result_ids.append(products_spu_id)
